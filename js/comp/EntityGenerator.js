...
export default class EntityGenerator extends Component{
    ...
    makeElements(attachment, widgetKey, items, cb){
    ...
    isGetMatchIndexEntity(widgetKey, itemIndex){
        let result = {
            isValid : false,
            widgetResource : null, 
        }

        const arrays = Array.from( this._resourceStore.get(widgetKey) );
        arrays.map(([key, value], itIndex) => {
            if(value._entityData.index === itemIndex){
                result.isValid = true;
                result.widgetResource = value.widget.widgetResource;
            }
        });
